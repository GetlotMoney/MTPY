...
| 命令 | `F:\Anaconda\envs\dvsr_gpu\python.exe train_VGSR_CUB.py --config experiments/02_ablation/ABL-006_disable_geometry_encoding/config.yaml` |
| 配置文件 | `experiments/02_ablation/ABL-006_disable_geometry_encoding/config.yaml` |
| 设备 | `cuda:0` |
| epoch | `30` |
...
- [ ] 创建 ABL-006 实验前 Git checkpoint。
- [ ] Codex 自审。
- [ ] Claude Code 固定三轮审查。
- [ ] 审查全部通过后运行训练。
- [ ] 复制训练日志到本实验 `logs/` 目录，并使用 `ABL-006_CUB_seed5_<YYYYMMDD-HHMMSS>.txt` 命名。
- [ ] 生成 `experiments/06_framework_flows/ABL-006_disable_geometry_encoding.md`，记录代码框架图、流程说明和本实验数据。
