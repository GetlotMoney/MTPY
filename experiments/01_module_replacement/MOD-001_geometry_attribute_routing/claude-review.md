...
- 实验配置可复现：
  - 独立配置文件：`experiments/01_module_replacement/MOD-001_geometry_attribute_routing/config.yaml`
  - 明确 seed：`random_seed=5`
  - 明确训练命令。
  - README 已列出日志副本位置：`experiments/01_module_replacement/MOD-001_geometry_attribute_routing/logs/`
...
```text
F:\Anaconda\envs\dvsr_gpu\python.exe train_VGSR_CUB.py --config experiments/01_module_replacement/MOD-001_geometry_attribute_routing/config.yaml
```
...
```text
experiments/01_module_replacement/MOD-001_geometry_attribute_routing/logs/
```
