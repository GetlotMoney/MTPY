...
- 实验记录：`experiments/05_ablation/ABL-005_disable_conditional_text/README.md`
- 实验配置：`experiments/05_ablation/ABL-005_disable_conditional_text/config.yaml`
- 基线配置：`config/VGSR_cub_gzsl.yaml`
- 审查输出：`experiments/05_ablation/ABL-005_disable_conditional_text/claude-review.md`
...
```powershell
$env:PYTHONIOENCODING='utf-8'; F:\Anaconda\envs\dvsr_gpu\python.exe train_VGSR_CUB.py --config experiments/05_ablation/ABL-005_disable_conditional_text/config.yaml
```
...
```text
experiments/05_ablation/ABL-005_disable_conditional_text/logs/ABL-005_CUB_seed5_<YYYYMMDD-HHMMSS>.txt
```
...
- 实验完成后是否会生成 `experiments/08_framework_flow_records/ABL-005_disable_conditional_text.md`，并包含流程图、流程说明、指标数据和日志路径？
