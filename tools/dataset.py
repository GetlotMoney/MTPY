...
class CUBDataLoader():
    def __init__(self, data_path, device, is_scale=False, is_unsupervised_attr=False, is_balance=True):
        ...
        # 构建 DataLoader (Windows 下 num_workers 必须为 0，否则多进程会卡死)
...
class SUNDataLoader():
    def __init__(self, data_path, device, is_scale=False, is_unsupervised_attr=False, is_balance=True):
        ...
        # 构建 DataLoader (Windows 下 num_workers 必须为 0，否则多进程会卡死)
        self.test_seen_loader = DataLoader(self.test_seen_dataset, batch_size=64, shuffle=False, num_workers=0)
        self.test_unseen_loader = DataLoader(self.test_unseen_dataset, batch_size=64, shuffle=False, num_workers=0)
...
class AWA2DataLoader():
    def __init__(self, data_path, device, is_scale=False, is_unsupervised_attr=False, is_balance=True):
        ...
        # 构建 DataLoader (Windows 下 num_workers 必须为 0，否则多进程会卡死)
        self.test_seen_loader = DataLoader(self.test_seen_dataset, batch_size=128, shuffle=False, num_workers=0)
        self.test_unseen_loader = DataLoader(self.test_unseen_dataset, batch_size=128, shuffle=False, num_workers=0)
