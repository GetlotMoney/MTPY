...
| 口径 | 配置 | U | S | H | ZSL |
|------|------|---:|---:|---:|---:|
| 当前 baseline / 当前主基线 | TUNE-004：局部补丁选择 + 几何感知编码 + 双向视觉-文本交互 + 条件文本扰动 + AG-JEPA 辅助训练，`lambda_topo_pearson=0.1`，seed=5 | 73.10 | 73.61 | 73.35 | 81.44 |
| 多 seed 参考 | 当前新 baseline 暂只有 seed=5 完整记录；后续需要 REV/TUNE 复核 seed=1/10 或用户指定 seed | 待复核 | 待复核 | 最高 H=73.35 | 待复核 |
...
注意：项目正式比较口径不是多 seed 均值，而是在预先记录的 seed 候选池中取主指标 H 的最大值。当前 baseline 已按用户确认提升为 TUNE-004，来源为 `lambda_topo_pearson=0.1` 的 seed=5 调参结果；后续仍需多 seed 复核稳定性。
