...
| `ABL-006` | open | 去掉几何感知编码 | 选中补丁后是否还需要位置关系建模 | `experiments/02_ablation/ABL-006_disable_geometry_encoding/config.yaml` | `F:\Anaconda\envs\dvsr_gpu\python.exe train_VGSR_CUB.py --config experiments/02_ablation/ABL-006_disable_geometry_encoding/config.yaml` | 最后一个待跑消融实验 |
